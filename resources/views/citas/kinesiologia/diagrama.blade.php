...
<head>
    ...
    <title>Diagrama Kinesiologia</title>
</head>
<body>
<br>
        ...
        <tr>
            @foreach($kinesiologos as $kinesiologo)
                @if($kinesiologo->user->status->id==1)
                    <td style="border:0.5px solid; padding: 1px; margin: 1px; font-size: 10px;">{{$kinesiologo->user->last_name}} {{$kinesiologo->user->name}}: <strong style="font-size: 12px">{{strtoupper($kinesiologo->cod_diagrama)}}</strong> </td>
                @endif
            @endforeach
        </tr>
    </table>
    ...
    <div class="row">
        ...
        <div class="col-sm">
            ...
            <select name="episodio">
                <option>Seleccione Episodio Activo</option>
